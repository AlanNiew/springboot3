...
public class JwtTokenUtil {
    ...
    public static Map<String,Object> buildJwt(String username){
        Map<String, Object> map = initClaims(username,false);
        String token = generateToken(username, map);
        map.put(ACCESS_TOKEN,token);
        map.put(REFRESH_TOKEN,refreshToken(token));
        return map;
    }
    ...
    public static String refreshToken(String oldToken) {
        ...
        return generateToken(username, initClaims(username,true));
    }
     ...
     * @return 负载集合
     */
    private static Map<String,Object> initClaims(String username,boolean isRefresh){
        Map<String, Object> claims = new HashMap<>();
        //"iss" (Issuer): 代表 JWT 的签发者。在这个字段中填入一个字符串，表示该 JWT 是由谁签发的。例如，可以填入你的应用程序的名称或标识符。
//        claims.put(Claims.ISSUER,"AlanNiew");
        ...
        claims.put(Claims.EXPIRATION,generatorExpirationDate(isRefresh));
        //"aud" (Audience): 代表 JWT 的接收者。这个字段可以填入该 JWT 预期的接收者，可以是单个用户、一组用户、或者某个服务。
//        claims.put(Claims.AUDIENCE,"User");
     ...
     * @return 预计失效时间
     */
    private static Date generatorExpirationDate(boolean isRefresh) {
        //预计失效时间为：token生成时间+预设期间, 如果刷新token，则失效时间为：token生成时间+预设期间*2
        if (isRefresh){
            return new Date(System.currentTimeMillis() + EXPIRATION_TIME * 2);
        }
